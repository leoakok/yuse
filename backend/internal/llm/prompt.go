...
func buildAgentSystemPrompt(assistantContext model.AssistantContextInput, twinContext string, githubConnected bool, githubLogin string) string {
	...
	case model.AssistantViewResumes:
		b.WriteString("On Resumes list. Tailor requests: research the job (fetch_url, web_search), compare requirements to Twin + CV (list_twin_entries, get_resume_content), ask ONE question per missing skill before adding it, then create_resume and populate with confirmed facts. Never output section headings or a markdown CV in chat.\n")
