...
func seedKnowledgeEntries(m *Memory, now time.Time) {
	...
	entries := []*model.KnowledgeEntry{
		...
		},
		{
			...
			Category: model.AssistantCategoryJobApplication,
			Tags:     []string{"job-application", "tailoring", "skill-gap", "role-tailor"},
			Body:     "Research the role, compare to Twin/CV, ask before claiming missing skills, assess fit honestly, tailor with new role-specific items (not visibility toggles alone), write a cover letter when tracking, save the application.",
			Enabled:  true,
		},
		{
			ID:       "kb-role-tailor",
			Slug:     "role-tailor-workflow",
			Title:    "Role-target CV workflow",
			Category: model.AssistantCategoryJobApplication,
			Tags:     []string{"role-tailor", "job-application", "cv-best-practices", "skill-gap"},
			Body:     "Know the user first (Twin + existing CVs), web_search the role, assess fit with reasons, duplicate or create, add_section_item for tailored SUMMARY and new experience/project items when bullets must differ per role, verify visible content before confirming.",
			Enabled:  true,
		},
		{
			ID:       "kb-ats-tips",
