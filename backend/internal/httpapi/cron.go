...
// LinkedInApplicationsCron syncs applied jobs for users with saved LinkedIn sessions.
func LinkedInApplicationsCron(pool *pgxpool.Pool, cronSecret string) http.HandlerFunc {
	syncer := linkedinsync.ApplicationSyncer{Pool: pool}
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet && r.Method != http.MethodPost {
			http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
			return
		...
		processed, err := syncer.SyncDueUsers(r.Context(), 20)
		resp := map[string]any{"processed": processed}
		if err != nil {
			resp["error"] = err.Error()
			w.Header().Set("Content-Type", "application/json")
