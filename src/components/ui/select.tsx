...
import { motionPopupEnter, motionPopupSlide, motionTransitionColors } from "@/lib/ui/motion"
import { cn } from "@/lib/utils"
import { ChevronDownIcon, CheckIcon, ChevronUpIcon } from "lucide-react"

type SelectItemEntry = {
  label: React.ReactNode
  value: React.ComponentProps<typeof SelectPrimitive.Item>["value"]
}

function resolveSelectItemLabel(
...
function collectSelectItems(children: React.ReactNode): SelectItemEntry[] {
  ...
  function walk(nodes: React.ReactNode) {
    React.Children.forEach(nodes, (child) => {
      ...
      if (child.type === SelectItem) {
        ...
        items.push({
          value: props.value,
          label: resolveSelectItemLabel(props.label, props.children),
        })
        return
      }
...
function Select({
  ...
  const items =
    itemsProp ?? (collectedItems.length > 0 ? collectedItems : undefined)
