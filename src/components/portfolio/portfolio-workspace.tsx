...
import type { PageFormat, SectionItem, SectionItemUsage } from "@/lib/types/cv";
...
import {
  PortfolioSectionItemEditDialog,
  type SectionItemDialogState,
} from "@/components/portfolio/portfolio-section-item-edit-dialog";
...
import {
  ...
  duplicatePortfolio,
  updatePortfolioSectionItemVisibility,
} from "@/lib/api/portfolio-api";
import { getSectionItemUsage } from "@/lib/api/cv-api";
...
export function PortfolioWorkspace({
  ...
  async function handleToggleVisibility(
    ...
    const updated = await updatePortfolioSectionItemVisibility(
  ...
  async function handleDuplicate() {
    ...
    try {
      const portfolio = await duplicatePortfolio(content.portfolio.id);
  ...
  async function handleDelete() {
    ...
    setIsDeleting(true);
    try {
      await deletePortfolio(content.portfolio.id);
  ...
  async function handleDeleteItem() {
    ...
    try {
      const updated = await deletePortfolioSectionItem(content.portfolio.id, deleteItemTarget.id);
      onContentChange(updated);
  ...
  function renderDeleteItemDescription() {
    ...
    const docs = [
      ...(deleteItemUsage?.resumes ?? []).map((r) => ({ id: r.id, title: r.title, kind: "resume" as const })),
      ...(deleteItemUsage?.portfolios ?? []).map((p) => ({ id: p.id, title: p.title, kind: "portfolio" as const })),
    ];
    ...
    if (count <= MAX_LISTED_PORTFOLIOS) {
      return (
        <>
          This item appears on {count} {docWord}:
    ...
    return (
      <>
        This item appears on {count} documents. It will be removed from all of them permanently.
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent showCloseButton={!isDeleting}>
          <DialogHeader>
            ...
            <DialogDescription>
              &ldquo;{content.portfolio.title}&rdquo; will be removed permanently. This cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
